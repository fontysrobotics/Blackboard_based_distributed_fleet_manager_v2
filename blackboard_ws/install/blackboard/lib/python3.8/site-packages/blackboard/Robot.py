...
class Robot:
    def __init__(self, bbAdress, backupAdress, robotId, robotType, repeatability, accuracy, payload, mxVelLinear, mxVelAngular, battery,nodeName,talker):
        ...
        rclpy.Subscriber('taskBC',TaskMsg,self.getTaskCost)                         # Ros subscribers
        rclpy.Subscriber('taskAssign',TaskMsg,self.addTask)                         #
        amclPose = '/'+self.nodeName+'/amcl_pose'                                    # topic name based on robot id
        rclpy.Subscriber(amclPose,PoseWithCovarianceStamped,self.initialPose)       # 
        self.bbBackupSub = rclpy.Subscriber('bbBackup',bbBackup,self.bbBackup)      #

        self.pingTimer = rclpy.Timer(rclpy.Duration(5),self.pingBlackboard)         # ros timers for function callback over duration
        self.bbBackupTimer = rclpy.Timer(rclpy.Duration(3),self.bbBackupActivate)   #
        self.exeTimer = rclpy.Timer(rclpy.Duration(1),self.executeTask)             #
